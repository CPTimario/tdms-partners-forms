...
vi.mock('@/components/support-form-builder/FillStep', () => ({
  FillStep: ({
    ...
    onRecipientSelect?: (item?: { id: string; label: string } | null) => void;
  }) => {
    return (
      <div>
        <button
          id="select"
          onClick={() =>
            onRecipientSelect && onRecipientSelect({ id: 'team::1', label: 'Southeast Team' })
          }
        >
          Select
        </button>
        <button id="clear" onClick={() => onRecipientSelect && onRecipientSelect(null)}>
          Clear
        </button>
      </div>
    );
...
describe('SupportFormBuilder URL sync', () => {
  ...
  test('adds recipient param when child selects a suggestion', async () => {
    const { SupportFormBuilder } = await import(
      '@/components/support-form-builder/SupportFormBuilder'
    );
    ...
    // flush pending updates then unmount the root to avoid createRoot warnings
    await act(async () => {
  ...
  test('initializes fields from deeplink recipient parameter', async () => {
    ...
    const crypto = await import('@/lib/deeplink-crypto');
    const token = crypto.encryptRecipient({
  ...
  test('auto-creates recipient token when required fields present (debounced)', async () => {
    ...
    const mockUseSupportForm = () => ({
      data: {
        membershipType: 'non-victory',
        missionaryName: 'Auto Test',
        nation: 'Nowhere',
        travelDate: localToday,
        sendingChurch: 'Test Church',
      },
      step: 'partner',
      fieldErrors: {},
      formErrors: [],
