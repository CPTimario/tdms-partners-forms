...
async function chooseMembership(page: Page, type: 'Victory Member' | 'Non-Victory Member') {
  ...
  if (type === 'Victory Member') {
    const victoryBtn = page
      .getByRole('button', { name: "Open partners' forms for Victory members", exact: true })
      .first();
    ...
    const nonVictoryBtn = page
      .getByRole('button', { name: "Open partners' forms for Non-Victory members", exact: true })
      .first();
  ...
  await expect(
    page.getByRole('heading', { name: 'Ten Days Missions Support Forms' }),
  ).toBeVisible();
}
...
async function fillPartnerStep(page: Page, currency: 'PHP' | 'USD' = 'USD') {
  ...
  const missionerInput = page
    .locator('label', { hasText: 'Missioner Name/Team' })
    .locator('input')
    .first();
  await missionerInput.fill('Southeast Team');
  await page.getByRole('textbox', { name: /^Nation/ }).fill('Thailand');
  const travelDateInput = page
    .locator('label', { hasText: 'Travel Date' })
    .locator('input')
    .first();
  ...
  const currencyTrigger = amountField
    .locator('button, [role="button"], [role="combobox"], .MuiSelect-select')
    .first();
  ...
  await amountField
    ...
    .evaluate((el: HTMLInputElement) => el.blur());
  ...
  await page
    .getByText('By providing my information, I am allowing Every Nation to process my information.')
    .click();
...
test.describe('Support forms end-to-end', () => {
  test('uses navigation semantics for step controls and updates active step', async ({
    page,
  }: {
    page: Page;
  }) => {
  ...
  test('membership CTAs are keyboard-focusable and route correctly', async ({
    page,
  }: {
    page: Page;
  }) => {
  ...
  test('keeps the victory agreement gate visible when Escape is pressed', async ({
    page,
  }: {
    page: Page;
  }) => {
  ...
  test('redirects to non-victory route and skips agreement gate', async ({
    page,
  }: {
    page: Page;
  }) => {
  ...
  test('supports direct non-victory route without membership gate', async ({
    page,
  }: {
    page: Page;
  }) => {
  ...
  test('keeps route context on reload for victory and non-victory entry points', async ({
    page,
  }: {
    page: Page;
  }) => {
  ...
  test('reload on victory route resets form data but keeps route context', async ({
    page,
  }: {
    page: Page;
  }) => {
  ...
  test('exposes only the I Agree action on the victory agreement gate', async ({
    page,
  }: {
    page: Page;
  }) => {
  ...
  test('requires membership selection and shows correct variant messaging', async ({
    page,
  }: {
    page: Page;
  }) => {
    ...
    await fillPartnerStep(page);
    await page.getByRole('button', { name: 'Continue to Accountability' }).click();
    const nonVictoryHeading = page.getByRole('heading', {
  ...
  test('blocks review until required fields are complete and then allows review', async ({
    page,
  }: {
    page: Page;
  }) => {
  ...
  test('toggles theme and persists selection on reload', async ({ page }: { page: Page }) => {
    ...
    const toggledTheme = (await page.evaluate(() => document.documentElement.dataset.theme)) as
      | string
      | undefined;
    ...
    await page
      .context()
      .addCookies([{ name: 'tdm-theme', value: themeToStore, domain: '127.0.0.1', path: '/' }]);
    await page.evaluate((v) => {
      try {
        window.localStorage.setItem('tdm-theme', v);
      } catch {}
    }, themeToStore);
  ...
  test('aria-pressed resolves to dark with no cookie when system preference is dark', async ({
    page,
  }: {
    page: Page;
  }) => {
  ...
  test('aria-pressed is correct after hydration when dark theme is set via cookie', async ({
    page,
  }: {
    page: Page;
  }) => {
  ...
  test('keeps selected theme while navigating between routes', async ({ page }: { page: Page }) => {
    ...
    await expect(toggle).toHaveAttribute(
      'aria-pressed',
      persistedTheme === 'dark' ? 'true' : 'false',
    );
  ...
  test('shows snackbar when continuing to accountability with invalid partner step', async ({
    page,
  }: {
    page: Page;
  }) => {
  ...
  test('shows snackbar when advancing to review with incomplete accountability step', async ({
    page,
  }: {
    page: Page;
  }) => {
  ...
  test('snackbar dismisses when the dismiss button is clicked', async ({
    page,
  }: {
    page: Page;
  }) => {
  ...
  test('snackbar timeout resets when shown again', async ({ page }: { page: Page }) => {
    ...
    const snackbar = page
      .locator('#mui-portal-root [role="alert"]', { hasText: 'Some fields need your attention' })
      .first();
  ...
  test('no snackbar shown when partner step is valid on continue', async ({
    page,
  }: {
    page: Page;
  }) => {
