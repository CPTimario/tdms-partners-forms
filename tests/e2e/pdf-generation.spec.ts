...
async function fillCompleteForm(page: Page, currency: 'PHP' | 'USD' = 'USD') {
  ...
  await page
    .getByRole('textbox', { name: 'Local Church', exact: true })
    .fill('Every Nation Makati');
  ...
  await page
    .getByRole('textbox', { name: 'Sending Church', exact: true })
    .fill('Every Nation Greenhills');
  ...
  await expect(
    page.getByRole('radio', { name: 'Redirect my support to the team fund' }),
  ).toBeChecked();
