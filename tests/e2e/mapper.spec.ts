...
test.describe('Development mapper', () => {
  test('renders mapper page with template background and repositions selected field on click', async ({
    page,
  }) => {
    await page.goto('/mapper');

    await expect(
      page.getByRole('heading', { name: 'PDF Mapper (Development Only)' }),
    ).toBeVisible();
  ...
  test('supports field selection, dragging, page switch, and keyboard nudging', async ({
    page,
  }) => {
    await page.goto('/mapper');
    ...
    const pageCombobox = page
      .locator('label', { hasText: 'Page' })
